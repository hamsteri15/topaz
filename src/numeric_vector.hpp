...
template <class T, class Allocator>
struct NumericVector : public vector_base_type<T, Allocator>{
...
public:
    ...
    inline NumericVector() = default;

    inline explicit NumericVector(size_type n, const value_type& value)
        : parent(n, value) {}

    template <class Iterator,
              typename = std::enable_if_t<IsIterator_v<Iterator>>>
    inline explicit NumericVector(Iterator begin, Iterator end)
        : parent(begin, end) {}

    inline explicit NumericVector(std::initializer_list<T> l) : parent(l.size()){
    ...
    inline NumericVector(const Range_t& rng)
        : parent(adl_begin(rng), adl_end(rng)) {}


    template <class Range_t,
              typename = std::enable_if_t<IsRangeOrNumericVector_v<Range_t>>>
    inline NumericVector& operator=(const Range_t& rng) {
